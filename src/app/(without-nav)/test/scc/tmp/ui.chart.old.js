  ...
  barDraw = {
    ...
    afterDraw: function (chart, args, options) {      
      ...
      function barDrawFocus(){
        ...
        //tooltip
        if(this.chart.toolTipTarget)
          this.chart.self.setDefaultTooltip(this.chart, 0, this.index);
      ...
      chart.$datalabels._labels.sort((a, b) => a._index - b._index ).some((itm,idx) => {
      //chart.$datalabels._labels.reverse().some((itm,idx) => {        
        ...
        bar.addEventListener('focus', barDrawFocus.bind({
          chart : chart,
          index: itm._index,
          bar : bar
        }));
      ...
      if(chart.data.labels.length > 0 ){
        ...
        chart.data.labels.some((label,idx)=>{
          ...
          if( !label === "" || label.startsWith("label") ) return false;
          ...
          ticks: {
            ...
            callback: function (value, index) {
              ...
              const text = Array.isArray(raw)
                ? raw.join("")
                : String(raw != null ? raw : value != null ? value : "");
  ...
  chartBarBaseLine(obj, json) {
    ...
    const nextDraw = {
      ...
      afterDraw: function (chart, args, options) {
        ...
        const avgValue =
          json.avgValue != null ? json.avgValue : allData.reduce((a, b) => a + b, 0) / allData.length;
  ...
  async chartDoughnut(obj, json) {
    ...
    async function makePatterns(ctx, colors) {
      function createSvgPattern(ctx, bgColor) {
        ...
        const svgUrl = `${gfn.devServer()?"":"/pconts"}/static/svg/chart/${_this.pattern[_svgCnt++]}.svg`;
    ...
    async function makeDoughnut(json, idx = 0) {
      ...
      if (dataTip && idx == 0) {
        ...
        data.labels.forEach((itm, $idx) => {
          ...
          img.onload = function() {
            const _width = 100;
            ctx.drawImage(img, 0, 0, _width, (img.height * (_width / img.width)));
          }
